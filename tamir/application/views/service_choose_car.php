						...
						<fieldset>
							...
							<div class="table-responsive">
								<table class="table">
									...
									<tbody>
		<?php
            $searchResult    = $this->db->query("SELECT * FROM cars WHERE plate_number LIKE '%$plate%' ORDER BY id DESC ");
            ...
            if ($searchRows > 0) {
                ...
                for ($s = 0; $s < count($searchData); $s++) {
                    ...
                    $car_plate        = $searchData[$s]->plate_number;
                    $car_make_id    = $searchData[$s]->car_make_id;
                    $car_model        = $searchData[$s]->car_model;
                    $car_owner_id    = $searchData[$s]->owner_id;
                    $car_mileage    = $searchData[$s]->mileage;
                    $car_color        = $searchData[$s]->color;
                    $car_trans        = $searchData[$s]->transmission;
                    $car_chassis    = $searchData[$s]->chassis_number;
					...
					<tr>
						...
						<td>
							<?php echo $car_mileage; ?>
						</td>
						<td>
							<?php echo $car_chassis; ?>
						</td>
						<td>
							<?php
                                if ($car_trans == "1") {

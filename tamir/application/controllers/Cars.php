...
class Cars extends CI_Controller
{
    ...
    public function insertCar()
    {
        ...
        if (empty($owner)) {
            ...
            $this->session->set_flashdata('alert_msg', array('failure', 'Add New Car', "Please Enter Car Chassis Number!"));
            redirect(base_url().'cars/add_car');
        } elseif (empty($color)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Add New Car', "Please Enter Car Color!"));
            redirect(base_url().'cars/add_car');
        } elseif (empty($mileage)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Add New Car', "Please Enter Car Mileage!"));
            redirect(base_url().'cars/add_car');
        } elseif (empty($transmission)) {
            $this->session->set_flashdata('alert_msg', array('failure', 'Add New Car', "Please Select Car Transmission!"));
            redirect(base_url().'cars/add_car');
        } else {
            $ckPlateResult    = $this->db->query("SELECT id FROM cars WHERE plate_number = '$plate' ");
            $ckPlateRows    = $ckPlateResult->num_rows();
            
            if ($ckPlateRows > 0) {
                $this->session->set_flashdata('alert_msg', array('failure', 'Add New Car', "Plate Number $plate is already existing in the system!"));
                redirect(base_url().'cars/add_car');
